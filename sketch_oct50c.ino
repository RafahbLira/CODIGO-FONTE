...
#define PINO_ZMPT101B 36 // Pino do sensor ZMPT101B 
...
void configuracaosensores() {
    ...
    //KY037:
    pinMode(PINO_KY037, INPUT); 

    //ZMPT101B
    pinMode(PINO_ZMPT101B, INPUT);
...
void ler_sensor_ZMPT101B() {
      ...
      valorinst = analogRead(PINO_ZMPT101B);
       if(menorvalor > valorinst){
      menorvalor = valorinst;
       }
      }
      Serial.print("Menor Valor: ");
      Serial.println(menorvalor);
      dados.tensaopico = map(menorvalor, 1850, 900, 0, 315);
      dados.tensaorms = dados.tensaopico/1.4;
}
